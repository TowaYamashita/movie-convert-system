...
export async function handler(event) {
  const outputFilePaths = getOutputObjectPaths(event);

  await updateInputS3ObjectTag(event);

  return {
    'output': outputFilePaths,
  }
}

// 出力先の動画がアップロードされたS3オブジェクトのバケット名とオブジェクトキーの組み合わせを取得する
function getOutputObjectPaths(event) {
  const outputGroupDetails = event.detail.outputGroupDetails;
  const outputDetails = outputGroupDetails.map((e) => e.outputDetails).flat();
  const outputFilePaths = outputDetails.map((e) => e.outputFilePaths).flat();
  return outputFilePaths.map((e) => parseS3Url(e));
}
